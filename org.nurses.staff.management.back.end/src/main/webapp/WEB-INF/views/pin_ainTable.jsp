...
<div style="text-align: center; width: 100%; margin:2px 0 2em;">
 ...
 <h1>PIN/AIN Table</h1>
 <hr class="d-sm" style="width:90%; margin:auto;">
...
<!-- <div style=" display:flex; flex-direction:row-reverse;"> <input class="form-control" id="myInput" type="text" placeholder="Search.." style="padding: .5em 2em; border-radius: 5em; width:20%; display:flex; flex-direction: flex-end;"> </div>
 -->        
  <div style=" display:flex; flex-direction:row-reverse;">    <button onclick="window.location.href='${pageContext.request.contextPath}/nurseFront/addStaff'" class="btn btn-primary" id="myInput" type="button"  style="padding: .5em 2em; border-radius: 5em; width:20%; display:; flex-direction: flex-end;">Add Pin/Ain </button>
 ...
 <select class  ="form-control" name="state" id="maxRows" style="width:20%; padding: .5em 2em; border-radius: 5em; display:flex; flex-direction: flex-end;">
            ...
            <c:forEach items="${pinGet}" var="pin">
            
            <c:url value="deletePin/${pin.id}" var="delete">
            </c:url>
            
            <c:url value="updatePin1/${pin.id}" var="update"></c:url>
            
                <tr>
                    <td>${pin.id}</td>
                    <td>${pin.staff_id}</td>
                    <td>${pin.pin_ain_number}</td>
                    <td>${pin.type}</td>
                    <td>${pin.date_issue}</td>
                    <td>${pin.renewal_date}</td>
                    <td>${pin.expire_date}</td>
                    <td id="action" class="w3-center">
                      <a id="action" href="${delete}" onclick="if (!confirm('Are you sure you want to delete this record?')) return false"> <i style="margin-right: 10px;" id="action" class="fa fa-trash"></i></a> |
