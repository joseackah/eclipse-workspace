...
<title>Display Students Address</title>
</head>
<body>
...
<div> <h2> <ins>Students Address List</ins></h2></div>
<div  ><input type="button" value="Enroll New Student" style='padding:.5em 2em; border-radius:5em; cursor:pointer;' onclick="window.location.href='${pageContext.request.contextPath}/schoolStudents/moreStudentsForm'" /></div>
		...
		<table border="1" id="name">
			...
			<tbody>
				<c:forEach items="${studentAddresses}" var="studentAddresses">
					<c:url value="update1" var="updateURL">
						<c:param value="${studentAddresses.studentAddId}" name="studentAddId"></c:param>
					 	<c:param value="${studentAddresses.town}" name="town"></c:param>
						<c:param value="${studentAddresses.city}" name="city"></c:param>
						<c:param value="${studentAddresses.postalCode}" name="postalCode"></c:param>
						<c:param value="${studentAddresses.resNumber}" name="resNumber"></c:param>
						
						 
						
					</c:url>

					<c:url value="deleteStudents/${studentAddresses.studentAddId}" var="removeURL">
				<!-- <c:param value="" name="id"></c:param> -->	
					</c:url>

					<tr>
						<td>${studentAddresses.studentAddId}</td>
						<td>${studentAddresses.town}</td>
						<td>${studentAddresses.city}</td>
						<td>${studentAddresses.postalCode}</td>
						<td>${studentAddresses.resNumber} </td>
